...
@section('title', 'Tambah Data Polres')
@section('page-title', 'Tambah Data Polres')
...
<div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden max-w-2xl">
    <div class="p-5 border-b border-outline-variant bg-surface-bright">
        <h3 class="font-display-sm text-display-sm text-primary font-semibold">Form Tambah Polres</h3>
    </div>
    
    <form action="{{ route('admin.polres.store') }}" method="POST" class="p-5 flex flex-col gap-4">
        @csrf
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="text" class="custom-input w-full h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface placeholder:text-outline-variant @error('nama_polres') border-error @enderror" id="nama_polres" name="nama_polres" value="{{ old('nama_polres') }}" required placeholder="Misal: Polres Metro Jakarta Selatan">
            @error('nama_polres')
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="text" class="custom-input w-full h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface placeholder:text-outline-variant @error('wilayah') border-error @enderror" id="wilayah" name="wilayah" value="{{ old('wilayah') }}" required placeholder="Misal: Polda Metro Jaya">
            @error('wilayah')
                <div class="text-error font-label-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
