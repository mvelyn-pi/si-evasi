...
<div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-hidden max-w-3xl">
    <div class="p-5 border-b border-outline-variant bg-surface-bright">
        <h3 class="font-display-sm text-display-sm text-primary font-semibold">Form Edit Pertanyaan</h3>
    </div>
    
    <form action="{{ route('admin.pertanyaan.update', $pertanyaan) }}" method="POST" class="p-5 flex flex-col gap-5" x-data="{ jenis: '{{ old('jenis_kuesioner', $pertanyaan->jenis_kuesioner) }}' }">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="flex flex-col gap-2">
                ...
                <div class="relative">
                    <select class="custom-input w-full h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface appearance-none cursor-pointer @error('jenis_kuesioner') border-error @enderror" id="jenis_kuesioner" name="jenis_kuesioner" x-model="jenis" required>
                        <option value="pemahaman" {{ old('jenis_kuesioner', $pertanyaan->jenis_kuesioner) == 'pemahaman' ? 'selected' : '' }}>Pemahaman Pengguna</option>
                        <option value="tam" {{ old('jenis_kuesioner', $pertanyaan->jenis_kuesioner) == 'tam' ? 'selected' : '' }}>Technology Acceptance Model (TAM)</option>
                        <option value="sus" {{ old('jenis_kuesioner', $pertanyaan->jenis_kuesioner) == 'sus' ? 'selected' : '' }}>System Usability Scale (SUS)</option>
                    </select>
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="number" class="custom-input w-full h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface placeholder:text-outline-variant @error('nomor_item') border-error @enderror" id="nomor_item" name="nomor_item" value="{{ old('nomor_item', $pertanyaan->nomor_item) }}" required>
                @error('nomor_item')
                    <div class="text-error font-label-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="relative">
                    <select class="custom-input w-full h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface appearance-none cursor-pointer @error('konstruk') border-error @enderror" id="konstruk" name="konstruk" :required="jenis === 'tam'">
                        <option value="Perceived Usefulness" {{ old('konstruk', $pertanyaan->konstruk) == 'Perceived Usefulness' ? 'selected' : '' }}>Perceived Usefulness</option>
                        <option value="Perceived Ease of Use" {{ old('konstruk', $pertanyaan->konstruk) == 'Perceived Ease of Use' ? 'selected' : '' }}>Perceived Ease of Use</option>
                        <option value="Attitude Toward Using" {{ old('konstruk', $pertanyaan->konstruk) == 'Attitude Toward Using' ? 'selected' : '' }}>Attitude Toward Using</option>
                        <option value="Behavioral Intention" {{ old('konstruk', $pertanyaan->konstruk) == 'Behavioral Intention' ? 'selected' : '' }}>Behavioral Intention</option>
                    </select>
                    <span class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-outline pointer-events-none">expand_more</span>
                </div>
                @error('konstruk')
        ...
        <div class="flex flex-col gap-2">
            ...
            <textarea class="custom-input w-full p-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface placeholder:text-outline-variant @error('teks_pertanyaan') border-error @enderror" id="teks_pertanyaan" name="teks_pertanyaan" rows="3" required>{{ old('teks_pertanyaan', $pertanyaan->teks_pertanyaan) }}</textarea>
        ...
        <template x-if="jenis === 'sus'">
            <div class="flex items-center gap-3 mt-1">
                ...
                <input class="w-4 h-4 text-primary bg-surface-container-lowest border-outline-variant focus:ring-primary rounded" type="checkbox" id="is_reverse" name="is_reverse" value="1" {{ old('is_reverse', $pertanyaan->is_reverse) ? 'checked' : '' }}>
        ...
        <div class="flex flex-col gap-2 mt-2">
            ...
            <div class="relative">
                <select class="custom-input w-full md:w-1/3 h-[40px] px-3 bg-surface-container-lowest border-[1.5px] border-outline-variant rounded-[6px] font-body-md text-on-surface appearance-none cursor-pointer @error('status') border-error @enderror" id="status" name="status">
                    <option value="aktif" {{ old('status', $pertanyaan->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="nonaktif" {{ old('status', $pertanyaan->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                </select>
